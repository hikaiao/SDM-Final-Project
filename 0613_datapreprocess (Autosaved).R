...
##Predict Power
p.r.f<-predict(r.f,newdata=testdata)
#varImpPlot(r.f)
#importance(r.f)
...
a<-errfinal(predictlabel,testdata1,labels=c("versicolor","virginica"),lastvarnum)
testerrtable<-tablefinal(a,r.f$classes)
testerrtable

##train error rate table
b<-errfinal(r.f$predict,traindata,labels=c("versicolor","virginica"),lastvarnum)
